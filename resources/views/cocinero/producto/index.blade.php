...
<div class="row">
	<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
		...
		@include('cocinero.producto.search')
	</div>
</div>
		...
		<div class="table-responsive">
			<table class="table table-striped table-bordered table-condensed table-hover">
				...
				<tr>
					...
					<td>{{$pro->precio}}</td>
					<td>
						<a href="{{URL::action('ProductoCController@edit',$pro->idproducto)}}"><button class="btn btn-info">Editar</button></a>
						<a href="" data-target="#modal-delete-{{$pro->idproducto}}" data-toggle="modal"><button class="btn btn-danger">Eliminar</button></a>
					</td>
				</tr>
				@include('cocinero.producto.modal')
				@endforeach
			</table>
		</div>
		{{$productos->render()}}
